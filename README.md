...
机器翻译每次只处理一个剧情帧。进入译文模式时只等待当前帧，阅读期间会在后台持续保持最多 10 个逻辑帧的预译缓冲；遇到未选择的分支时，各分支分别前进一帧合计为一个逻辑帧。对话框会显示“已译未读”缓冲数量，每次接口响应都会立即写入本地缓存。若翻译服务失败，当前帧会稳定显示日文原文，已返回的译文仍会在历史记录或再次访问时使用。
