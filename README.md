...
3. 如果已配置并选择翻译后端，可点击“一次性翻译并导出”。阅读器会翻译当前脚本的全部文本（包括所有选择分支），按 5 个剧情帧一组单线程顺序完成后直接下载已填充译文的人工翻译 JSON；已有机器或人工译文会复用，不完整的翻译不会导出。进度旁每 1 秒刷新一次基于本地译文文本估算的最近 1 秒平均 TPS，不依赖翻译接口返回 token 用量。
