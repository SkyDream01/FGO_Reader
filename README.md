...
机器翻译按五个剧情步骤一批处理。进入译文模式时只等待当前位置起的首批五步，随后按“当前批次、临近选项与下一批、当前路线、其他分支”的顺序继续后台翻译；每次接口响应都会立即写入本地缓存。连续点击、自动播放、跳读或按住 `Ctrl` 快进不会等待后续批次，若阅读速度追上翻译队列，该帧会稳定显示日文原文，已返回的译文将在历史记录或再次访问时使用。
