...
机器翻译以 2 路并发请求运行，每次请求只处理一个剧情帧。进入译文模式时只等待当前帧，阅读期间会在后台持续保持最多 10 个逻辑帧的预译缓冲；遇到未选择的分支时，各分支分别前进一帧合计为一个逻辑帧。对话框会显示“已译未读”缓冲数量，每次接口响应都会立即写入本地缓存。若翻译服务失败，当前帧会稳定显示日文原文，已返回的译文仍会在历史记录或再次访问时使用。
...
3. 如果已配置并选择翻译后端，可点击“一次性翻译并导出”。阅读器会翻译当前脚本的全部文本（包括所有选择分支），分批完成后直接下载已填充译文的人工翻译 JSON；已有机器或人工译文会复用，不完整的翻译不会导出。
