...
ID 的结构（也对应头部 `＄` 标签）可以稳定拆成 `2-2-2-2-1-1` 六段；各段的业务含义只能作为当前目录的观察结果，不能当作公开的引擎接口：

| 位置 | 含义 | 示例值 |
|------|------|--------|
| XX | 大类别（观察值） | 01 包含特异点/终局，02 包含伪特异点，03 包含序章/异闻带/部分活动，04 包含终局后与 Ordeal Call 等，05 为特殊章节 |
| XX | 章节号 | 00, 01... |
| XX | 子章节/任务号 | 03, 06, 07... |
| XX | 场景/阶段号 | 01, 02... |
| X | 分支/变体 | 0, 1, 2... |
...
以上拆解是基于文件名和目录的观察结果，不应视为引擎公开定义。当前样本中 `93` 出现在少量头部而不是文件名首段，属于历史/兼容数据。
...
角色的 `X,Y` 使用最终合成画面中心作为 `(0,0)`：`X` 负值向左、正值向右；`Y` 正值向上、负值向下。坐标按原始 1024×576 舞台换算，角色缩放不会改变坐标单位；角色最终合成的坐标锚点位于画面中心。
