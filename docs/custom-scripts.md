...
每条对话以 <code>＠</code> 开头；解析器也兼容半角 <code>@</code>，韩服脚本通常使用半角形式。一条对话必须用 <code>[k]</code>、<code>[page]</code> 或 <code>[q]</code> 结束；否则解析器会在下一结构处自动结束，并给出诊断。

最简单的署名形式是 <code>＠玛修</code>。也可以让槽位成为发言者：<code>＠MASH：玛修</code>（冒号可使用全角或半角）；前者是槽位名，后者是显示名称。<code>[scene]</code> 的 ID 和 <code>charaSet</code> 的角色 ID 通常为数字；它们会作为字符串与 <code>assets</code> 中的映射键匹配，因此自定义包也可以使用非数字键，只要剧本与清单保持一致。<code>[bgm]</code> 的 ID 不能含空格，后面的可选参数会被阅读器忽略。
...
| <code>[charaFadein 槽位 时长 X,Y]</code>、<code>[charaFadein 槽位 时长 1]</code> | 淡入角色；传入坐标时使用指定位置，传入位置编号 <code>1</code> 或省略坐标时位于中心 <code>(0,0)</code>。 |
...
阅读器只实现常用的 ADV 命令子集。不要把剧情推进建立在未列出的游戏命令上；不支持的命令会保留为解析诊断，但不会改变当前画面。<code>charaFace</code> 会被解析；回退到 Atlas 远程立绘的角色会正常叠加表情差分，但本地映射的角色图片是单张图片，不会随表情切换变化。

## 选项、收束与重置

选项标题由问号、数字、冒号和选项文字组成，例如 <code>？1：继续前进</code>。全角 <code>？1：</code> 与半角 <code>?1:</code> 都受支持，韩服脚本通常使用半角形式；结束行同样可写成 <code>？！</code> 或 <code>?!</code>。选项内部不能再嵌套选项，选项文字不能留空。分支正文可以为空，此时选择后会直接进入选项组后的共同剧情。
...
所有选项应在结束标记前收束到同一剧情状态。解析器只会继承所有分支完全一致的场景、BGM、发言槽和角色状态；存在分歧的字段会回到进入选项前的状态，并产生诊断。因此若任一分支改过背景、音乐或角色，请在选项组之后明确重置为共同状态：
...
当 <code>region</code> 为 <code>JP</code> 时，导入预览会显示“允许此脚本使用翻译服务”开关，默认关闭。即使已开启，读者仍需在阅读器设置中选择翻译后端，再用 <code>T</code> 或工具栏的“译文”按钮切换；只有进入译文模式后文本才会发送给其选定后端。以后可在“浏览脚本库”中切换该包的“译文”开关来撤回**未来**授权；这无法撤回已经发送给第三方后端的文本。需要清除本机翻译记录时，可在阅读设置中分别清除本地凭据和机器翻译缓存；人工译文需要单独移除。请在导入前告知读者这一点，并确认你有权将相关内容交给所选翻译服务处理。
...
| 清单或脚本无效 | 重新用 UTF-8 保存；检查固定字段、版本号、<code>script</code> 文件名、对话起始标记和结束标记。 |
...
| 提示具体行列的解析错误 | 检查未闭合的方括号、空选项文字、缺失的选项结束标记、嵌套选项或超过限制的结构。未知命令只产生警告，不会阻止导入。 |
...
- 对话使用 <code>＠</code>（韩服可用 <code>@</code>），选项使用一致的全角或半角标点，每条对话都有 <code>[k]</code>、<code>[page]</code> 或 <code>[q]</code>。
- 选项不嵌套，分支结束后重置到共同状态。
