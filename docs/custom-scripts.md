...
选项标题由问号、数字、冒号和选项文字组成，例如 <code>？1：继续前进</code>。全角 <code>？1：</code> 与半角 <code>?1:</code> 都受支持，韩服脚本通常使用半角形式；结束行同样可写成 <code>？！</code> 或 <code>?!</code>。选项内部不能再嵌套选项，选项文字和分支正文也不能留空。
...
| 提示具体行列的解析错误 | 检查未闭合的方括号、空选项、缺失的选项结束标记、嵌套选项或超过限制的结构。未知命令只产生警告，不会阻止导入。 |
